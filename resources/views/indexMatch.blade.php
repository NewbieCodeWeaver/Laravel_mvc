...
@section('content')
  ...
  <div id="body" class="py-20 px-6 lg:py-20 lg:px-14 bg-slate-200 bg-stadium bg-cover">
    <div class="grid">
      <div class="justify-self-center lg:justify-self-end w-42 rounded-lg bg-red-600 mb-8 p-4 text-center text-base font-semibold text-red-50 hover:bg-red-800">
        <a href="{{route('partido.add')}}"> <i class="fa-solid fa-circle-plus"></i></i>
          <span class="text-center uppercase">añadir partido</span>
        </a>
      </div>
    </div>
    <div id="partidos-table" class="overflow-x-auto">
      <table class="w-full">
        ...
        <tbody>
          @foreach ($partidos as $partido)  
            <tr class=" bg-teal-50">
              ...
              <td class="text-base text-gray-900 font-light px-10 py-4 whitespace-nowrap pb-5 ">
                @if ( $date < $partido->fecha ) 
                  <i>Pendiente<i>
                @else {{$partido->resultado}}
                @endif
              </td>
              <td class=" text-base text-gray-900 font-light px-10 py-4 whitespace-nowrap pb-5 ">
